...
class RamanDataDigitizer:
    def __init__(self, root):
        ...
        # --- State Variables ---
        self.image = None
        self.image_array = None # Stores the ORIGINAL image array (numpy)
        self.photo = None
        self.original_image = None  # Store original for zoom operations
        self.zoom_level = 1.0  # Track current zoom level
        
        # All coordinates are stored relative to the ORIGINAL image size
    ...
    def apply_zoom(self):
        ...
        if self.zoom_level != 1.0:
            ...
            self.image = self.original_image.copy() # Use copy to avoid modifying original
        
        # Update display photo (do NOT update image_array, it keeps original)
    ...
    def process_new_image(self, img):
        try:
            ...
            self.image = self.original_image.copy()   # Working copy (displayed)
            self.zoom_level = 1.0
            if hasattr(self, 'zoom_label'):
                self.zoom_label.config(text="100%")
            # Keep original array for calculations
    ...
    def draw_current_line_mask(self):
        ...
        for x, y in zip(x_coords[::5], y_coords[::5]):  # Sample every 5th point for performance
            # SCALED DRAWING
            sx, sy = x * self.zoom_level, y * self.zoom_level
            self.canvas.create_oval(sx-1, sy-1, sx+1, sy+1, fill=color, outline='', tags="line_mask")
    ...
    def on_canvas_click(self, event):
        ...
        # Map canvas click to original image coordinates
        x_raw, y_raw = self.canvas.canvasx(event.x), self.canvas.canvasy(event.y)
        x = int(x_raw / self.zoom_level)
        y = int(y_raw / self.zoom_level)
        ...
        if self.current_mode == "pick_color": 
            ...
            # Draw temp rect in CANVAS coordinates for visual feedback
            self.temp_rect_id = self.canvas.create_rectangle(x_raw, y_raw, x_raw, y_raw, outline='red', width=2, dash=(5,3))
    ...
    def on_canvas_motion(self, event):
        ...
        x_raw, y_raw = self.canvas.canvasx(event.x), self.canvas.canvasy(event.y)
        self.mouse_x = int(x_raw / self.zoom_level)
        self.mouse_y = int(y_raw / self.zoom_level)
    ...
    def on_canvas_drag(self, event):
        ...
        if self.current_mode == "adjust_calibration" and self.dragging_line_index is not None:
            ...
            # Convert start to canvas coords for drawing
            sx0, sy0 = x0 * self.zoom_level, y0 * self.zoom_level
            self.canvas.coords(self.temp_rect_id, sx0, sy0, x_raw, y_raw)
    ...
    def on_canvas_release(self, event):
        if self.current_mode == "adjust_calibration": self.escape_mode()
            ...
            x_raw, y_raw = self.canvas.canvasx(event.x), self.canvas.canvasy(event.y)
            x1 = int(x_raw / self.zoom_level)
            y1 = int(y_raw / self.zoom_level)
    ...
    def detect_text_regions(self):
        ...
        # ... (Same logic, simplified for brevity, assume works on 'gray')
        # [Preserving existing logic but ensuring it uses the 'gray' derived from original image]
        try:
            ...
            for contour in contours:
                ...
                area = w * h
                aspect_ratio = w / h if h > 0 else 0
                if (area > 100 and area < 50000 and aspect_ratio > 0.2 and aspect_ratio < 20 and w > 10 and h > 5):
                    padding = 5
                    x = max(0, x - padding)
                    y = max(0, y - padding)
                    w = min(self.original_image.width - x, w + 2*padding)
                    h = min(self.original_image.height - y, h + 2*padding)
        ...
        try:
            ...
            for contour in contours:
                x, y, w, h = cv2.boundingRect(contour)
                area = w * h
                if (area > 50 and area < 10000 and w > 8 and h > 8 and w < 200 and h < 100):
        ...
        try:
            ...
            for i in range(1, num_labels):
                x, y, w, h, area = stats[i]
                if (area > 20 and area < 1000 and w > 3 and w < 50 and h > 5 and h < 50):
                    aspect_ratio = w / h if h > 0 else 0
            ...
            if digit_regions:
                ...
                for i in range(1, len(digit_regions)):
                    ...
                    if x_distance < 20 and y_distance < 10:
                        ...
                        if len(current_group) >= 1:
                            ...
                            max_x = min(self.original_image.width, max_x + padding)
                            max_y = min(self.original_image.height, max_y + padding)
    ...
    def calculate_overlap_area(self, region1, region2):
        ...
        if x2_min <= x1_max or y2_min <= y1_max: return 0
    ...
    def filter_spectral_line_regions(self, regions):
        ...
        """
        if len(self.calibration_points) != 4: return regions
        ...
        if hasattr(self, 'extracted_points') and self.extracted_points:
            for point in self.extracted_points:
                ...
                spectral_path_regions.append((
                    ...
                    min(self.original_image.width, x + padding), min(self.original_image.height, y + padding)
                ))
        
        filtered = []
        for region in regions:
            region_area = (region[2] - region[0]) * (region[3] - region[1])
            if region_area > 0.3 * calib_area: continue
            ...
            for spectral_region in spectral_path_regions:
                ...
                if overlap > 0:
                    ...
                    break
            if overlaps_spectral_data: continue
            filtered.append(region)
        return filtered
    ...
    def auto_exclude_borders(self):
        ...
        try:
            ...
            # Method 1: Strong edge detection
            try:
                ...
                for contour in contours:
                    x, y, w, h = cv2.boundingRect(contour)
                    area = w * h
                    if (area > 10000 and w > 100 and h > 100 and area < 0.8 * self.original_image.width * self.original_image.height):
                        perimeter = cv2.arcLength(contour, True)
                        approx = cv2.approxPolyDP(contour, 0.02 * perimeter, True)
                        if len(approx) >= 4:
                            border_thickness = 8
                            # Add 4 sides
                            detected_borders.append((max(0, x-border_thickness), max(0, y-border_thickness), min(self.original_image.width, x+w+border_thickness), min(self.original_image.height, y+border_thickness*2))) # Top
                            detected_borders.append((max(0, x-border_thickness), max(0, y+h-border_thickness*2), min(self.original_image.width, x+w+border_thickness), min(self.original_image.height, y+h+border_thickness))) # Bottom
                            detected_borders.append((max(0, x-border_thickness), max(0, y-border_thickness), min(self.original_image.width, x+border_thickness*2), min(self.original_image.height, y+h+border_thickness))) # Left
                            detected_borders.append((max(0, x+w-border_thickness*2), max(0, y-border_thickness), min(self.original_image.width, x+w+border_thickness), min(self.original_image.height, y+h+border_thickness))) # Right
            except Exception as e:
            ...
            try:
                ...
                if lines is not None:
                    ...
                    for line in lines:
                        ...
                        if (angle_deg < 10 or angle_deg > 170): horizontal_lines.append((rho, theta))
                        elif (80 < angle_deg < 100): vertical_lines.append((rho, theta))
                    
                    border_thickness = 6
                    for rho, theta in horizontal_lines[:10]:
                        a = np.cos(theta); b = np.sin(theta)
                        x0 = a * rho; y0 = b * rho
                        y_line = int(y0)
                        if 0 < y_line < self.original_image.height:
                            detected_borders.append((0, max(0, y_line-border_thickness), self.original_image.width, min(self.original_image.height, y_line+border_thickness)))
                    for rho, theta in vertical_lines[:10]:
                        a = np.cos(theta); b = np.sin(theta)
                        x0 = a * rho; y0 = b * rho
                        x_line = int(x0)
                        if 0 < x_line < self.original_image.width:
                            detected_borders.append((max(0, x_line-border_thickness), 0, min(self.original_image.width, x_line+border_thickness), self.original_image.height))
            except Exception as e:
            ...
            messagebox.showinfo("Border Exclusion Complete", f"Added {len(filtered_borders)} border exclusion zones.")
    ...
    def update_zoom_preview(self):
        if self.image is None: return
        size = 50 # Size of area to crop from original image
        ...
        # Crop from ORIGINAL image (high res)
        zoom_region = self.original_image.crop((x_start, y_start, x_end, y_end))
        zoom_enlarged = zoom_region.resize((150, 150), Image.Resampling.NEAREST)
        
        overlay_img = Image.new('RGBA', (150, 150), (0, 0, 0, 0))
        draw = ImageDraw.Draw(overlay_img)
        
        # Draw exclusion zones (coordinates are already in original scale)
        ...
        # Draw extracted points (coordinates are already in original scale)
        ...
        self.zoom_canvas.delete("all")
        self.zoom_canvas.create_image(75, 75, image=self.zoom_photo)
    ...
    def select_color_at_point(self, x, y):
        # x, y are already original image coordinates
    ...
    def auto_calibrate(self):
        ...
        try:
            ...
            w, h = self.original_image.size; margin = min(w, h) // 10
    ...
    def draw_calibration_points(self):
        ...
        for i, (x, y) in enumerate(self.calibration_points):
            ...
            sx, sy = x * self.zoom_level, y * self.zoom_level
            self.canvas.create_oval(sx-5, sy-5, sx+5, sy+5, fill=self.calib_color, outline='black', width=1, tags="calibration_points")
    ...
    def draw_calibration_grid(self):
        ...
        for i in range(4):
            ...
            # Scale coordinates
            sx1, sy1 = p1[0] * self.zoom_level, p1[1] * self.zoom_level
            sx2, sy2 = p2[0] * self.zoom_level, p2[1] * self.zoom_level
            self.canvas.create_line(sx1, sy1, sx2, sy2, fill=self.calib_color, width=2, dash=(5,3), tags="calibration_grid")

    def get_line_at_pos(self, x, y, tolerance=5):
        # Input x, y are original coordinates
        ...
        # If tolerance is 5 pixels on screen, then on image it is 5 / zoom_level
        adjusted_tolerance = tolerance / self.zoom_level
        ...
        for i, (p1, p2) in enumerate(lines):
            ...
            if np.dot(p3 - p1, p2 - p1) >= 0 and np.dot(p3 - p2, p1 - p2) >= 0:
                ...
                if dist < adjusted_tolerance: return i
        return None
    ...
    def draw_exclusion_zones(self):
        ...
        for (x0, y0, x1, y1) in self.exclusion_zones:
            # Scale coordinates
            sx0, sy0 = x0 * self.zoom_level, y0 * self.zoom_level
            sx1, sy1 = x1 * self.zoom_level, y1 * self.zoom_level
            self.canvas.create_rectangle(sx0, sy0, sx1, sy1, fill='red', stipple='gray25', outline='', tags="exclusion")
    ...
    def extract_data_dual(self):
        """Run both detection methods in a background thread."""
        ...
        # (Simplified for now, ideally would disable specific buttons) 
        
        def processing_task():
            try:
                # Run Method 1
                self.root.after(0, lambda: self.progress_bar.configure(value=25))
                self.method1_points = self.extract_data_method1()
                
                # Run Method 2
                self.root.after(0, lambda: self.progress_bar.configure(value=75))
                self.method2_points = self.extract_data_method2()
                
                self.root.after(0, lambda: self.progress_bar.configure(value=100))
                
                # Finished, update UI on main thread
                self.root.after(0, self.finish_extraction)
                
            except Exception as e:
                self.root.after(0, lambda: messagebox.showerror("Extraction Error", f"Failed during data extraction.\nError: {e}"))
                self.root.after(0, lambda: self.progress_bar.pack_forget())

        threading.Thread(target=processing_task, daemon=True).start()

    def finish_extraction(self):
        """Called when background extraction thread completes."""
    ...
    def draw_extracted_points(self):
        ...
        for x, y in self.extracted_points:
            # Scale coordinates
            sx, sy = x * self.zoom_level, y * self.zoom_level
            self.canvas.create_oval(sx-2, sy-2, sx+2, sy+2, fill=point_color, outline="", tags="extracted")
    ...
    def draw_extrema_points(self):
        ...
        # Scale coordinates
        px_sx, px_sy = peak_px_x * self.zoom_level, peak_px_y * self.zoom_level
        vx_sx, vx_sy = valley_px_x * self.zoom_level, valley_px_y * self.zoom_level
        
        self.canvas.create_oval(px_sx-5, px_sy-5, px_sx+5, px_sy+5, fill=self.peak_color, outline='black', width=1, tags='extrema')
        self.canvas.create_oval(vx_sx-5, vx_sy-5, vx_sx+5, vx_sy+5, fill=self.valley_color, outline='black', width=1, tags='extrema')
    ...
    def draw_best_fit_line(self):
        ...
        if self.best_fit_line_pixel_coords and self.show_best_fit.get():
            ...
            # Scale coordinates
            sx1, sy1 = x1 * self.zoom_level, y1 * self.zoom_level
            sx2, sy2 = x2 * self.zoom_level, y2 * self.zoom_level
            self.canvas.create_line(sx1, sy1, sx2, sy2, fill='purple', width=2, tags="best_fit")
